...
-export([
   ...
   , getWork/1             %% get a worker
   , getWork/2             %% get a worker
...
openW(WName, Kvs) ->
   ...
   case supervisor:start_child(eFaw_sup, FChildSpec) of
      {ok, _Pid} = Ret ->
         ...
         [supervisor:start_child(WName, [WorkName, worker, WName:getV(?wArgs)]) || {_Idx, WorkName} <- NameList],
...
getWork(WName) ->
   getWork(WName, bind).
getWork(WName, Strategy) ->
   WFCnt = WName:getV(?wFCnt),
   Idx = ?IIF(Strategy == rand, erlang:unique_integer([positive]) rem WFCnt + 1, erlang:phash2(self(), WFCnt) + 1),
...
hireW(WorkerNum, FName, IsTmp) when is_integer(WorkerNum), WorkerNum > 0 ->
   ...
   case supervisor:start_child(FName, [FWorkerName, IsTmp, FName:getV(?wArgs)]) of
      {ok, _Pid} ->
         ignore;
      ErrRet ->
